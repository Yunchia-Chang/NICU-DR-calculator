...
b1_bw = st.sidebar.number_input("請輸入體重 (kg)", min_value=0.0, max_value=10.0, value=0.0, step=0.1)
...
l1_pna = st.sidebar.number_input("請輸入出生天數 (days)", min_value=0, max_value=365, value=0, step=1)
...
with col1:
    st.metric(label="👶 病患體重 (BW)", value=f"{b1_bw} kg" if b1_bw > 0 else "未輸入")
with col2:
    st.metric(label="⏳ 胎齡 (GA)", value=f"{f1_ga_wk} 週 + {f1_ga_day} 天" if f1_ga_wk > 0 else "未輸入")
with col3:
    if q1_pma_wk > 0:
        st.success(f"🧮 **PMA (受孕齡) 系統換算結果 [Q1+S1]**：\n### {q1_pma_wk} 週 + {s1_pma_day} 天")
    else:
        st.info("請於側邊欄輸入完整 GA 與 PNA 以換算 PMA")
...
with tab1:
    ...
    # 安全防錯檢查：檢查必要欄位是否填寫
    if b1_bw == 0 or f1_ga_wk == 0:
        st.warning("⚠️ 請先於左側欄位輸入「體重 (BW)」與「胎齡 (GA)」，系統將自動計算建議劑量。")
    else:
        # 1. 轉化您的 Excel 劑量公式 (B4)
        ga_days = (f1_ga_wk * 7) + h1_ga_day
        
        if ga_days <= 244:  # GA ≦ 34+6wk
            if l1_pna <= 7:
                b4_dose = b1_bw * 50
                is_valid = True
            else:
